...
// Заменяем millis() на реализацию для Linux
uint32_t millis() {
...
void esp_log_writev(int level, const char* tag, const char* format, va_list args) {
  printf("[%s] ", tag);
  vprintf(format, args);
  printf("\n");
}
...
bool IsoTp::send_sf(Message_t& msg)  // Send SF Message
{
  ...
  return can_send(msg.tx_id, msg.len + 1, TxBuf);  // Send only required bytes (PCI + data)
}
