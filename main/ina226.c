...
static i2c_device_config_t dev_cfg = {.dev_addr_length = I2C_ADDR_BIT_LEN_7,
...
static esp_err_t ina226_write_reg(uint8_t reg, uint16_t value) {
  ...
  esp_err_t err =
      i2c_master_transmit(dev_handle, data, sizeof(data), pdMS_TO_TICKS(1000));
...
static esp_err_t ina226_read_reg(uint8_t reg, uint16_t *value) {
  ...
  esp_err_t err = i2c_master_transmit_receive(
      dev_handle, &write_data, 1, read_data, 2, pdMS_TO_TICKS(1000));
  if (err == ESP_OK) {
...
void ina226_init(const ina226_config_t *config) {
  ...
  uint16_t reg_value;
  const uint8_t regs[]    = {INA226_REG_CONFIG,
                             INA226_REG_SHUNT_VOLT,
                             INA226_REG_BUS_VOLT,
                             INA226_REG_POWER,
                             INA226_REG_CURRENT,
                             INA226_REG_CALIB,
                             INA226_REG_ManufID};
  const char *reg_names[] = {"CONFIG",
                             "SHUNT_VOLT",
                             "BUS_VOLT",
                             "POWER",
                             "CURRENT",
...
esp_err_t ina226_read_values(float *voltage, float *current, float *power) {
  ...
  // ESP_ERROR_CHECK(ina226_read_reg(INA226_REG_SHUNT_VOLT,
  // (uint16_t*)&shunt_voltage));
  ESP_ERROR_CHECK(ina226_read_reg(INA226_REG_BUS_VOLT, &bus_voltage));
  ESP_ERROR_CHECK(
      ina226_read_reg(INA226_REG_CURRENT, (uint16_t *)&current_raw));
  ESP_ERROR_CHECK(ina226_read_reg(INA226_REG_POWER, &power_raw));
